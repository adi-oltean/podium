...
mode, for the transcendental functions as well; matrix products agree up to
...
preservation from C to assembly for the supported C subset and calling
convention, and on a second instruction-set architecture (ISA). The golden
vectors thus tie the Python reference, the emitted C, and the compiled assembly
...
end: a convex planner produces the approach, the plan is propagated through the
nonlinear truth model with $J_2$ and differential drag, the closed-loop
trajectory is checked against approach-corridor and keep-out Signal Temporal
Logic specifications, and the passive-drift abort safety (the safe free-drift
trajectory after a thruster cutoff) is certified in exact rational arithmetic by
the barrier. The exact-rational KKT and conic-KKT checkers, which re-verify an
online QP or SOCP solve against its own optimality conditions, run in continuous
integration as part of the verification suite (\texttt{test\_kkt.py}) rather than
inside the reference-mission loop. An interactive playback of the same burn plan
under the nonlinear truth model is hosted at
...
implementation; further rendezvous-specific successive-convexification work
includes factorization-free \cite{chari2024} and continuous-time
temporal-logic \cite{uzun2026} formulations. Prong (i) for RPOD is thus
