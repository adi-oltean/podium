...
Frama-C Evolved Value Analysis (EVA) plug-in \cite{framac2015}, and
...
checker that computes an exact rational bound on the solution's suboptimality. The
gap between a Python prototype and
...
correctly rounded implementations of sine and cosine from CORE-MATH
...
Repository \texttt{github.com/adi-oltean/podium}; language Python 3.10+; license
...
Exact-rational KKT certificates & online QP solves given an exact rational suboptimality bound, SOCP solves re-verified against conic-KKT, over $\Q$ \\
...
checked in exact rational arithmetic over the rationals $\Q$, with no floating
...
error. The emitted flight C is tied to the Python reference by the golden vectors
(above) and carried to assembly by CompCert, so the validated algorithm and the
compiled algorithm agree on the tested inputs. Every tagged
...
under the nonlinear truth model is hosted, as a current project demo, at
\url{https://adi-oltean.github.io/podium/} as static HTML
...
layer. On certificates, RealCertify \cite{magron2018}, the Coq-checked ValidSDP
\cite{validsdp}, and verified SDP \cite{jansson2009} produce exact-rational or
rigorously bounded certificates for general polynomial and conic problems, and
certifiable trajectory optimization \cite{strom2024}, certifiable perception
\cite{yang2023perception}, and exact SDP relaxations
...
contract-annotated C, and execution-time-certified embedded QP solvers
\cite{eiqp2025} bound the online iteration count; verified spacecraft control
...
certificates with an RPOD library. The simulators Basilisk
