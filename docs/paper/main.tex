...
Spacecraft rendezvous and docking place strong demands on software. The maneuvers
...
optimization is now standard in guidance \cite{acikmese2007,malyuta2022}, but a
...
\draw[->] (emit) -- (cc);
\draw[->] (cc) -- (asm);
\node[box, above=7mm of emit, xshift=9mm] (eva) {Frama-C/EVA};
\draw[->, dashed] (emit.north) |- (eva.west) node[pos=0.7, above, font=\footnotesize] {};
...
state-transition matrix, the Yamanaka--Ankersen matrix for elliptic orbits
\cite{yamanaka2002}, relative orbital elements with Keplerian, $J_2$, and
...
checker: a barrier for abort safety, KKT conditions for the online convex solves,
and bounds on the global optimum of the nonconvex docking subproblem. The
...
\texttt{examples/vbar\_approach.py}, plans a one-kilometer approach along the
