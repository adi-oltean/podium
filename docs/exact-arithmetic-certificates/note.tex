...
\texttt{github.com/adi-oltean/podium}. AI coding assistants were used for
drafting, for the reference implementation and verification scripts, and for
adversarial review; all results were checked by the author. Cite as
\url{https://doi.org/10.5281/zenodo.21247380}.}}
...
\begin{abstract}
Guidance and control software relies on numerical optimization and
floating-point synthesis whose executions are difficult to verify directly. We
study the problem of verifying solver \emph{outputs}: given a certificate
produced by an untrusted floating-point solver, we re-verify it in exact rational
arithmetic, so that acceptance constitutes a proof that carries no rounding
error. This note documents the exact-rational certificates in the Podium library
(\texttt{podium.verify}) and applies the method to four properties: abort safety,
via barrier certificates; optimality of the online convex solves, via
Karush--Kuhn--Tucker conditions for quadratic and second-order cone programs;
closed-loop stability, via Lyapunov ellipsoids from a discrete-time
linear-quadratic regulator; and polynomial set containment, via sum-of-squares
certificates. These certificates are standard; what we give is exact
re-verification --- tolerance-free for all but the KKT re-check of an
approximate online solve, which yields an exact suboptimality bound --- together
with their prior-art positioning and a reference implementation in which every
result is exercised by a test.

For nonconvex QCQPs we specialize the exact rational recovery of a semidefinite
certificate, developed for polynomial optimization by Peyrl and Parrilo and by
Kaltofen et al., to the S-procedure dual. This yields a tolerance-free,
machine-checkable optimality-gap bracket: a rational lower bound from the dual
and a rational upper bound from a feasible point, coinciding exactly at a global
optimum. Our analytic contribution characterizes how that lower bound loses
value under rational rounding of the dual multiplier --- at second order in the
rounding denominator in the nonsingular case, at first order in the trust-region
hard case, sound in both, and vanishing when the optimal multiplier is rational
(Theorems~\ref{thm:recovery} and~\ref{thm:hard}). For several constraints the
S-procedure need not be tight; the bracket then encloses $J^\star$ between exact
rational bounds that need not coincide, as on the Celis--Dennis--Tapia instance
where $[-3,\,-17520/11881]$ bounds both the suboptimality of the feasible point
and the duality gap (Theorem~\ref{thm:multi}).
\end{abstract}
...
verifier rather than on the solver. This note constructs such certificates for
...
the global optimum. This note supplies such a bound: an exact-arithmetic
...
actuator signals. Verifying the \emph{execution} of these components is possible
only in narrow settings --- for active-set quadratic-programming methods on
multiparametric problems, the exact worst-case iteration count can be certified
offline \cite{arnstrom2022} --- and such analyses are specific to one solver and
one problem family, leaving the floating-point behavior of the routines
themselves uncovered. We instead verify the \emph{answer}: an
...
yields a certified upper bound on the suboptimality of the feasible point, and hence on the duality gap, when the S-procedure is not tight
...
nonnegative multiplier certifies that one quadratic is nonnegative on the region
where another is nonnegative. The barrier is expressed on a basis of quantities
conserved by the passive drift, so its Lie derivative --- the rate of change of
the barrier along the drift --- vanishes by construction. Together these
establish forward invariance of the safe set, so that the abort set never
intersects the keep-out zone \cite{prajna2004,ames2017}.
...
solution by the rational rounding of Peyrl and Parrilo \cite{peyrl2008}.
...
Suppose $A(\lambda^\star) \succ 0$ and $\lambda^\star > 0$ is an interior
maximizer of the dual. Then $g$ is smooth and concave near
...
Suppose $A(\lambda^\star)$ is positive semidefinite but singular, $b\in
\mathrm{range}\,A(\lambda^\star)$, and the limiting minimizer
$\bar x=\lim_{\lambda\downarrow\lambda^\star}\bigl(-\tfrac12A(\lambda)^{-1}b\bigr)$
satisfies $f_1(\bar x)\ne0$. When $\ker A(\lambda^\star)$ is invariant under $P_1$ ---
in particular in the trust-region case $P_1=-I$ --- one has
$\bar x=-\tfrac12A(\lambda^\star)^\dagger b$ and $f_1(\bar x)>0$ is the classical
hard case \cite{moresorensen1983}. Then:
...
need not hold, as $J^\star$ may be algebraic of degree greater
than one.
\end{theorem}
\begin{proof}
...
$\lambda^\star$ on that boundary. A boundary maximizer need not be stationary; here the right derivative
$g'_+(\lambda^\star) := \lim_{\lambda \downarrow \lambda^\star} g'(\lambda) =
-f_1(\bar x)$ is nonzero by hypothesis --- by Danskin, $g'(\lambda)=-f_1(x^\star(\lambda))$
along the smooth branch --- and a first-order expansion gives
...
recovery in the singular case remains open.

\subsection{Several constraints}

\begin{theorem}[Certified bound on the duality gap]\label{thm:multi}
...
of $t$ over $\lambda \ge 0$ and $t$ with $M(\lambda, t) \succeq 0$. Then
...
endpoints exact rationals; $f_0(\bar x) - t$ is then a certified upper bound on
the suboptimality of $\bar x$, and hence on the duality gap $J^\star - d^\star$.
\end{theorem}

\begin{proof}
The lower-bound argument of Theorem~\ref{thm:sound} uses only that a multiplier
$\lambda \ge 0$ with $M(\lambda, t) \succeq 0$ makes
$f_0(x) - \sum_{k=1}^{m} \lambda_k f_k(x) - t$ a global quadratic minorant; it
does not use $m = 1$. For any feasible $x$ one has $f_k(x) \ge 0$ and
$\lambda_k \ge 0$, so
\[
  f_0(x) - t \;\ge\; \sum_{k=1}^{m} \lambda_k\, f_k(x) \;\ge\; 0,
\]
hence $t \le f_0(x)$ for every feasible $x$, and therefore $t \le J^\star$.
Taking the supremum over $\lambda \ge 0$ and $t$ with
$M(\lambda, t) \succeq 0$ gives $d^\star \le J^\star$.

That the inequality can be strict for $m \ge 2$ is classical: exactness of the
S-procedure is a two-quadratic phenomenon---the S-lemma holds for a single
constraint but not in general for two or more \cite{polik2007}---so the Shor
relaxation of a multi-constraint QCQP need not be tight; the
Celis--Dennis--Tapia family below furnishes instances with $d^\star < J^\star$.

Finally, let $t$ be a certified lower bound, so $\lambda \ge 0$ and
$M(\lambda, t) \succeq 0$ pass the exact PSD test, and let $\bar x$ be exactly
feasible, so $f_k(\bar x) \ge 0$ for every $k$, each checked over $\Q$. Then
$t \le d^\star \le J^\star \le f_0(\bar x)$, so $J^\star \in [t, f_0(\bar x)]$
with both endpoints exact rationals, and
$f_0(\bar x) - t \ge J^\star - d^\star \ge 0$ is a certified upper bound on both
the suboptimality of $\bar x$ and the duality gap.
\end{proof}
...
The implementation exhibits the strict case on the two-ball Celis--Dennis--Tapia
instance $\min 2x_1x_2$ subject to $\lVert x-(1,0)\rVert \le 2$ and
$\lVert x+(1,0)\rVert \le 2$,\footnote{The
...
can be inexact.} a trust-region hard case in which the largest certifiable lower
bound is $d^\star = -3$. What the checker establishes over $\Q$ is the enclosure:
the bracket $[-3,\,-17520/11881]$ contains this nonconvex optimum, which is
irrational, with both endpoints verified exactly, so the suboptimality of the
feasible point and the duality gap are bounded above by
$-17520/11881 - (-3)$. That the gap is strictly positive here --- that
$d^\star < J^\star$ --- is a known property of the instance, observed
numerically ($J^\star \approx -1.476$); no exact computation in this note
certifies it, and Theorem~\ref{thm:multi} bounds the gap only from above.
...
subproblem \cite{elango2025}. Such methods return a feasible
...
a feasible point, hence a certified upper bound $u = f_0(\bar x)$; the
S-procedure dual of Section~\ref{sec:bracket} supplies a certified lower bound
$t \le J^\star$ for the same problem (Theorem~\ref{thm:sound}); and $[t, u]$ is a
...
$\min -x_1^2{+}2x_2^2{+}2x_2$, $\lVert x\rVert\le1$ & singular & $1$ & $t=-4/3$ & $-4/3$ \\
two active keep-outs & several constraints & $(\tfrac12,\tfrac12)$ & $[5,\,9]$ & $5$ \\
Celis--Dennis--Tapia $\min 2x_1x_2$ & duality gap & $(\tfrac12,\tfrac12)$ & $[-3,\,-\tfrac{17520}{11881}]$ & ${\approx}\,{-}1.476$ \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Mechanization.} Theorem~\ref{thm:sound}, the lower-bound leg of
Theorem~\ref{thm:multi} with its closing bracket observation, and the scalar case
of Lemma~\ref{lem:schur} have machine-checked counterparts in a Lean~4 corpus
(\texttt{note\_thm\_sound}, \texttt{note\_thm\_multi}, \texttt{note\_bracket},
\texttt{note\_lem\_schur}). The companion note~\cite{oltean2026leannote} maps each
declaration to its result here and records the scope limits: the mechanization
takes the PSD-to-minorant lifting step as a hypothesis rather than proving it, and
the note's remaining results are not mechanized.
...
(Theorem~\ref{thm:multi}) remain open. The $LDL^\top$ PSD test is $O(n^3)$; on the
per-primitive problems these checkers target, the rational entries stay small, but
they grow with problem size under exact elimination, which is why the certificates
are applied per primitive rather than to arbitrary sizes.
...
hard case, with soundness retained in both, and a certified upper bound on the
suboptimality gap when the relaxation is not tight. The constructions are
realized in an open reference implementation.
