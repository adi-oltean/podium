...
with both endpoints exact rationals, and $f_0(\bar x) - t \ge 0$ is a certified
bound on the duality gap; when it is positive it certifies a nonzero gap exactly.
\end{proof}
...
The implementation exhibits the strict case on a two-constraint instance of
Celis--Dennis--Tapia type,\footnote{The
...
can be inexact.} for which the largest certifiable lower bound lies strictly below
the optimum; the bracket then encloses a nonconvex optimum with no closed form,
with both endpoints verified over $\Q$.
