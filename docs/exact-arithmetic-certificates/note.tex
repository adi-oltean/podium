...
\texttt{github.com/adi-oltean/podium}. This technical note documents the
exact-arithmetic certificates in \texttt{podium.verify} and their proofs; the
underlying certificate mathematics is standard, with a modest original analysis of
the rational-rounding degradation (see the positioning at the end of
Section~\ref{sec:intro}). AI coding assistants were used for drafting, for the
reference implementation and verification scripts, and for adversarial review; all
results were checked by the author. Theorem~\ref{thm:sound}, the lower-bound leg of
Theorem~\ref{thm:multi} with its closing bracket observation, and the scalar case of
Lemma~\ref{lem:schur} have machine-checked counterparts in a Lean~4 corpus
...
declaration to its result here and records its scope limits --- in particular, the
mechanization takes the PSD-to-minorant lifting step as a hypothesis rather than
proving it, and the note's remaining results are not mechanized. Cite as
...
encloses $J^\star$ between exact rational bounds, certifying a strictly positive
suboptimality gap on a Celis--Dennis--Tapia instance (Theorem~\ref{thm:multi}). Each
...
certifies a strictly positive suboptimality gap when the S-procedure is not tight
...
bound $d^\star = -3$ lies strictly below the optimum $J^\star \approx -1.476$; the
bracket $[-3,\,-17520/11881]$ then encloses this nonconvex optimum, which is
irrational, with both endpoints verified over $\Q$.
...
hard case, with soundness retained in both, and a certified strictly positive
