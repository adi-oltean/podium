...
tolerance-free instantiation, its verification, and its relation to the
...
certificates. These certificates are standard; we give an exact, tolerance-free
re-verification procedure and a reference implementation.
...
for the online convex solves below, the residuals are still computed exactly, and
acceptance requires them to lie within an explicit rational tolerance that then
bounds the error.
...
program (SOCP) online. Since the solver returns an approximate primal-dual point,
the checker computes the KKT residuals exactly, stationarity, primal and dual
feasibility, and complementary slackness, and accepts the point when each lies
within an explicit rational tolerance $\varepsilon$; membership of a slack vector
$s = (s_0, s_{1:}) \in \R \times \R^d$ in the second-order cone is decided without
square roots through the algebraic condition $s_0 \ge -\varepsilon$ and
$\lVert s_{1:}\rVert^2 \le (s_0 + \varepsilon)^2$. Because the residuals are
exact, $\varepsilon$ is a certified bound on the point's suboptimality and
infeasibility. The checker also verifies that the objective Hessian is PSD, so
that the certificate concerns a global optimum of a convex program rather than a
stationary point of unknown type.
