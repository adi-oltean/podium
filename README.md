...
Mature tools exist — Basilisk, NASA 42, Trick, Orekit, GMAT — and rpod-lib does not re-fight their battles (see the [comparative analysis](docs/comparative-analysis.md)). What none of them offer together:
...
- **Truth vs. flight separation:** truth models may use anything in SciPy; flight algorithms live in `rpod.core` under the static-subset rules ([`docs/verification.md`](docs/verification.md)) and are exercised in the sim through the same step-function interface they will have in C.
- **Determinism:** fixed-step master clock, seeded noise, no wall-clock or platform dependence in results.
- **Contracts as data:** every core function declares input ranges/invariants via `rpod.verify`; checked at runtime in the sandbox, emitted as annotations for the external prover in the C translation.
...
[Apache-2.0](LICENSE) — permissive with an explicit patent grant, compatible with research and commercial flight-software use.
